...
public class FreecellModel implements FreecellOperations {
  ...
  public void move(PileType source, int pileNumber,
    ...
    List<LinkedList<Cards>> source_pile = new ArrayList<LinkedList<Cards>>();

    // get card from foundation pile. first check pile type.
    if (source.equals(PileType.FOUNDATION)) {
      source_pile.addAll(this.foundationPiles.getPiles());
      card_shifting = getShiftingCard(source_pile, pileNumber, cardIndex);
    }

    // get card from open pile. first check pile type.
    if (source.equals(PileType.OPEN)) {
      source_pile.addAll(this.openPiles.getPiles());
      card_shifting = getShiftingCard(source_pile, pileNumber, cardIndex);
    }

    // get card from cascade pile. first check pile type.
    if (source.equals(PileType.CASCADE)) {
      source_pile.addAll(this.cascadePiles.getPiles());
      card_shifting = getShiftingCard(source_pile, pileNumber, cardIndex);
    }
